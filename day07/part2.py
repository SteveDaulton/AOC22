...
def calculate(data):
    ...
    used_size = max([di.get_size() for di in DIRS.values()])
    needed = used_size - TOFREE
    return min([filebytes for di in DIRS.values()
                if (filebytes := di.get_size()) > needed])


DIRS = {Pp('/'): Dir(Pp('/'))}
TOFREE: int = 40000000

print(f'Answer = {calculate("input")}')  # 272298
